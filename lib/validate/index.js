...
class Validate {
  ...
  _validate(input, pattern) {
    ...
    if (_typeof(input) === 'object' && Object.keys(input).length > 0) {
      ...
      for (const key in pattern) {
        ...
        if (_typeof(pv) === 'string') {
          results[key] = that._string(input[key], pv);
        } else if (Array.isArray(pv)) {
          results[key] = that._array(input[key], pv);
        } else if (_typeof(pv) === 'object' && Object.keys(pv)) {
          results[key] = that._object(input[key], pv);
          input[key] = results[key].output;
        } else {
      ...
      } else if (_typeof(pattern) === 'object' && Object.keys(pattern)) {
  ...
  _string(value, pattern) {
    ...
    if (result.isValid !== true) {
      result.error = `invalid type, provided "${_typeof(value)}", expected "${pattern}"`;
    }
    return result;
  }
